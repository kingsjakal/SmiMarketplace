...
namespace Smi.Web.Areas.Admin.Controllers
{
    public partial class CustomerController : BaseAdminController
    {
        ...
        protected virtual string ValidateCustomerRoles(IList<CustomerRole> customerRoles, IList<CustomerRole> existingCustomerRoles)
        {
            ...
            if (rolesToAdd.Any(role => role.SystemName != SmiCustomerDefaults.RegisteredRoleName) || rolesToDelete.Any())
            {
            ...
            var isInGuestsRole = customerRoles.FirstOrDefault(cr => cr.SystemName == SmiCustomerDefaults.GuestsRoleName) != null;
            var isInRegisteredRole = customerRoles.FirstOrDefault(cr => cr.SystemName == SmiCustomerDefaults.RegisteredRoleName) != null;
        ...
        {
            var customers = _customerService.GetAllCustomers(customerRoleIds: new[] { _customerService.GetCustomerRoleBySystemName(SmiCustomerDefaults.AdministratorsRoleName).Id });
        ...
        public virtual IActionResult Create(CustomerModel model, bool continueEditing, IFormCollection form)
        {
            ...
            if (newCustomerRoles.Any() && newCustomerRoles.FirstOrDefault(c => c.SystemName == SmiCustomerDefaults.RegisteredRoleName) != null &&
            ...
            if (ModelState.IsValid)
            {
                ...
                if (_customerSettings.DateOfBirthEnabled)
                    _genericAttributeService.SaveAttribute(customer, SmiCustomerDefaults.DateOfBirthAttribute, model.DateOfBirth);
                ...
                foreach (var customerRole in newCustomerRoles)
                {
                    ...
                    if (customerRole.SystemName == SmiCustomerDefaults.AdministratorsRoleName && !_customerService.IsAdmin(_workContext.CurrentCustomer))
                        continue;
                ...
                {
                    var vendorRole = _customerService.GetCustomerRoleBySystemName(SmiCustomerDefaults.VendorsRoleName);
            ...
            if (ModelState.IsValid)
            {
                try
                {
                        ...
                        if (!string.IsNullOrEmpty(model.VatNumber))
                        {
                            if (!model.VatNumber.Equals(prevVatNumber, StringComparison.InvariantCultureIgnoreCase))
                            {
                                _genericAttributeService.SaveAttribute(customer,
                                    SmiCustomerDefaults.VatNumberStatusIdAttribute,
                    ...
                    foreach (var customerRole in allCustomerRoles)
                    {
                        ...
                        if (customerRole.SystemName == SmiCustomerDefaults.AdministratorsRoleName &&
                        ...
                        else
                        {
                            ...
                            if (customerRole.SystemName == SmiCustomerDefaults.AdministratorsRoleName && !SecondAdminAccountExists(customer))
                            {
        ...
        public virtual IActionResult Impersonate(int id)
        {
            ...
            _customerService.UpdateCustomer(customer);
            _genericAttributeService.SaveAttribute<int?>(_workContext.CurrentCustomer, SmiCustomerDefaults.ImpersonatedCustomerIdAttribute, customer.Id);
        ...
        public virtual IActionResult SendEmail(CustomerModel model)
        {
            ...
            try
            {
                if (string.IsNullOrWhiteSpace(customer.Email))
                    throw new SmiException("Customer email is empty");
                if (!CommonHelper.IsValidEmail(customer.Email))
                    throw new SmiException("Customer email is not valid");
                if (string.IsNullOrWhiteSpace(model.SendEmail.Subject))
                    throw new SmiException("Email subject is empty");
                if (string.IsNullOrWhiteSpace(model.SendEmail.Body))
                    throw new SmiException("Email body is empty");

                var emailAccount = _emailAccountService.GetEmailAccountById(_emailAccountSettings.DefaultEmailAccountId);
                if (emailAccount == null)
                    emailAccount = _emailAccountService.GetAllEmailAccounts().FirstOrDefault();
                if (emailAccount == null)
                    throw new SmiException("Email account can't be loaded");
        ...
        public virtual IActionResult SendPm(CustomerModel model)
        {
            ...
            try
            {
                if (!_forumSettings.AllowPrivateMessages)
                    throw new SmiException("Private messages are disabled");
                if (_customerService.IsGuest(customer))
                    throw new SmiException("Customer should be registered");
                if (string.IsNullOrWhiteSpace(model.SendPm.Subject))
                    throw new SmiException(_localizationService.GetResource("PrivateMessages.SubjectCannotBeEmpty"));
                if (string.IsNullOrWhiteSpace(model.SendPm.Message))
                    throw new SmiException(_localizationService.GetResource("PrivateMessages.MessageCannotBeEmpty"));
        ...
        public virtual IActionResult LoadCustomerStatistics(string period)
        {
            ...
            var searchCustomerRoleIds = new[] { _customerService.GetCustomerRoleBySystemName(SmiCustomerDefaults.RegisteredRoleName).Id };
