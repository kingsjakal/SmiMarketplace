CREATE FUNCTION [Smi_splitstring_to_table]
(
    @string NVARCHAR(MAX),
    @delimiter CHAR(1)
)
...
BEGIN
	...
	SELECT CAST(data as int) FROM [Smi_splitstring_to_table](@CategoryIds, ',')	
	...
	SELECT CAST(data as int) FROM [Smi_splitstring_to_table](@AllowedCustomerRoleIds, ',')
	...
	SELECT CAST(data as int) FROM [Smi_splitstring_to_table](@FilteredSpecs, ',') 
...
BEGIN
	--create catalog
	EXEC('
	IF NOT EXISTS (SELECT 1 FROM sys.fulltext_catalogs WHERE [name] = ''SmiMarketplaceFullTextCatalog'')
		CREATE FULLTEXT CATALOG [SmiMarketplaceFullTextCatalog] AS DEFAULT')
	...
	IF NOT EXISTS (SELECT 1 FROM sys.fulltext_indexes WHERE object_id = object_id(''[Product]''))
		...
		KEY INDEX [' + @index_name +  '] ON [SmiMarketplaceFullTextCatalog] WITH CHANGE_TRACKING AUTO'
...
BEGIN
	...
	--drop catalog
	EXEC('
	IF EXISTS (SELECT 1 FROM sys.fulltext_catalogs WHERE [name] = ''SmiMarketplaceFullTextCatalog'')
		DROP FULLTEXT CATALOG [SmiMarketplaceFullTextCatalog]
	')
END
GO
