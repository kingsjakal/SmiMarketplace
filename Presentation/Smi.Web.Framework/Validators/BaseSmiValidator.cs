...
namespace Smi.Web.Framework.Validators
{
    /// <summary>
    /// Base class for validators
    /// </summary>
    /// <typeparam name="TModel">Type of model being validated</typeparam>
    public abstract class BaseSmiValidator<TModel> : AbstractValidator<TModel> where TModel : class
    {
        #region Ctor

        protected BaseSmiValidator()
        {
            PostInitialize();
        }
        ...
        protected virtual void SetDecimalMaxValue(EntityDescriptor entityDescriptor)
        {
            ...
            foreach (var expression in maxValueExpressions)
            {
                RuleFor(expression.Expression).IsDecimal(expression.MaxValue)
                    .WithMessage(string.Format(localizationService.GetResource("Smi.Web.Framework.Validators.MaxDecimal"), expression.MaxValue - 1));
            }
        }
