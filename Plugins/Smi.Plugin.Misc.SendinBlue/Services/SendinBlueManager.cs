...
using Smi.Plugin.Misc.SendinBlue.Domain;
    ...
    public class SendinBlueManager
    {
        ...
        private TClient CreateApiClient<TClient>(Func<Configuration, TClient> clientCtor) where TClient : IApiAccessor
        {
            ...
            if (string.IsNullOrEmpty(sendinBlueSettings.ApiKey))
                throw new SmiException($"Plugin not configured");
                ...
                foreach (var storeId in storeIds)
                {
                    ...
                    var csv = subscriptions.Aggregate(title, (all, subscription) =>
                    {
                        ...
                        if (customer != null)
                        {
                            ...
                            phone = _genericAttributeService.GetAttribute<string>(customer, SmiCustomerDefaults.PhoneAttribute);
                            ...
                            gender = _genericAttributeService.GetAttribute<string>(customer, SmiCustomerDefaults.GenderAttribute);
                            dateOfBirth = _genericAttributeService.GetAttribute<string>(customer, SmiCustomerDefaults.DateOfBirthAttribute);
                            ...
                            zipCode = _genericAttributeService.GetAttribute<string>(customer, SmiCustomerDefaults.ZipPostalCodeAttribute);
                            ...
                            state = _stateProvinceService.GetStateProvinceById(_genericAttributeService.GetAttribute<int>(customer, SmiCustomerDefaults.StateProvinceIdAttribute))?.Name;
                            fax = _genericAttributeService.GetAttribute<string>(customer, SmiCustomerDefaults.FaxAttribute);
                        }
                        return $"{all}\n" +
                            $"{subscription.Email};" +
                            $"{firstName};" +
        ...
        public int? GetTemplateId(int? templateId, MessageTemplate message, EmailAccount emailAccount)
        {
            try
            {
                ...
                //or create new one
                if (emailAccount == null)
                    throw new SmiException("Email account not configured");
        ...
        public QueuedEmail GetQueuedEmailFromTemplate(int templateId)
        {
            try
            {
                ...
                if (templateId == 0)
                    throw new SmiException("Message template is empty");
        ...
        public void SendSMS(string to, string from, string text)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(to) || string.IsNullOrEmpty(text))
                    throw new SmiException("Phone number or SMS text is empty");
        ...
        public string SendSMSCampaign(int listId, string from, string text)
        {
            try
            {
                ...
                if (listId == 0 || string.IsNullOrEmpty(text) || string.IsNullOrEmpty(from))
                    throw new SmiException("List or SMS text or sender name is empty");
