...
using Smi.Core.Http;
using Smi.Plugin.Widgets.GoogleAnalytics.Api;
...
namespace Smi.Plugin.Widgets.GoogleAnalytics
{
    public class EventConsumer : IConsumer<OrderCancelledEvent>, IConsumer<OrderPaidEvent>, IConsumer<EntityDeletedEvent<Order>>
    {
        ...
        private void ProcessOrderEvent(Order order, bool add)
        {
            try
            {
                ...
                request.SendRequest(trans, _httpClientFactory.CreateClient(SmiHttpDefaults.DefaultHttpClient));
            }
            catch (Exception ex)
            {
