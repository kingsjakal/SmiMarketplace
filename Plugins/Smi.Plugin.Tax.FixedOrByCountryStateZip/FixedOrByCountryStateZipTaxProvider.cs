...
using Smi.Plugin.Tax.FixedOrByCountryStateZip.Infrastructure.Cache;
...
namespace Smi.Plugin.Tax.FixedOrByCountryStateZip
{
    /// <summary>
    ...
    public class FixedOrByCountryStateZipTaxProvider : BasePlugin, ITaxProvider
    {
        ...
        {
            if (!(_httpContextAccessor.HttpContext.Items.TryGetValue("Smi.TaxTotal", out var result) && result is TaxTotalResult taxTotalResult))
            {
                ...
                _httpContextAccessor.HttpContext.Items.TryAdd("Smi.TaxTotal", taxTotalResult);
            }
            ...
            if (taxTotalRequest.UsePaymentMethodAdditionalFee && _taxSettings.PaymentMethodAdditionalFeeIsTaxable)
            {
                var paymentMethodSystemName = taxTotalRequest.Customer != null
                    ? _genericAttributeService.GetAttribute<string>(taxTotalRequest.Customer,
                        SmiCustomerDefaults.SelectedPaymentMethodAttribute, taxTotalRequest.StoreId)
                    : string.Empty;
