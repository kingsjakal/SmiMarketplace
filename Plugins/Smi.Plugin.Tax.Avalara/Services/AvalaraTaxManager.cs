    ...
    public class AvalaraTaxManager : IDisposable
    {
        ...
        private TResult HandleFunction<TResult>(Func<TResult> function)
        {
            try
            {
                ...
                if (!IsConfigured())
                    throw new SmiException("Tax provider is not configured");
        ...
        private TransactionModel CreateTransaction(CreateTransactionModel model)
        {
            var transaction = ServiceClient.CreateTransaction(null, model)
                ?? throw new SmiException("No response from the service");
            ...
            if (transaction.messages?.Any() ?? false)
            {
                ...
                throw new SmiException(message);
            }
                ...
                if (_taxSettings.EuVatEnabled)
                {
                    ...
                    var useEuVatRules = (product?.IsTelecommunicationsOrBroadcastingOrElectronicServices ?? false)
                        && ((_countryService.GetCountryByAddress(billingAddress)
                            ?? _countryService.GetCountryById(_genericAttributeService.GetAttribute<int>(customer, SmiCustomerDefaults.CountryIdAttribute))
                            ?? _countryService.GetCountryByTwoLetterIsoCode(_geoLookupService.LookupCountryIsoCode(customer.LastIpAddress)))
                            ?.SubjectToVat ?? false)
                        && _genericAttributeService.GetAttribute<int>(customer, SmiCustomerDefaults.VatNumberStatusIdAttribute) != (int)VatNumberStatus.Valid;
        ...
        {
            return HandleFunction(() => ServiceClient.Ping() ?? throw new SmiException("No response from the service"));
        }
        ...
        public int? ExportTaxCodes()
        {
            return HandleFunction<int?>(() =>
            {
                if (string.IsNullOrEmpty(_avalaraTaxSettings.CompanyCode) || _avalaraTaxSettings.CompanyCode.Equals(Guid.Empty.ToString()))
                    throw new SmiException("Company not selected");
                ...
                var selectedCompany = GetAccountCompanies()
                    ...
                    ?? throw new SmiException("Failed to retrieve company");
        ...
        public bool DeleteSystemTaxCodes()
        {
            return HandleFunction(() =>
            {
                ...
                _cacheManager.Remove(SmiTaxDefaults.TaxCategoriesAllCacheKey);
            ...
            return HandleFunction(() =>
            {
                ...
                if (_shippingSettings.AllowPickupInStore)
                {
                    var pickupPoint = _genericAttributeService
                        .GetAttribute<PickupPoint>(customer, SmiCustomerDefaults.SelectedPickupPointAttribute, taxTotalRequest.StoreId);
                    if (pickupPoint != null)
                    {
                ...
                order.CheckoutAttributesXml = _genericAttributeService
                    .GetAttribute<string>(customer, SmiCustomerDefaults.CheckoutAttributes, taxTotalRequest.StoreId);

                //shipping method
                order.ShippingMethod = _genericAttributeService
                    .GetAttribute<ShippingOption>(customer, SmiCustomerDefaults.SelectedShippingOptionAttribute, taxTotalRequest.StoreId)?.Name;
                ...
                if (taxTotalRequest.UsePaymentMethodAdditionalFee)
                {
                    order.PaymentMethodSystemName = _genericAttributeService
                        .GetAttribute<string>(customer, SmiCustomerDefaults.SelectedPaymentMethodAttribute, taxTotalRequest.StoreId);
