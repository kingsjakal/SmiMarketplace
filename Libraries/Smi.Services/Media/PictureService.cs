...
namespace Smi.Services.Media
{
    ...
    public partial class PictureService : IPictureService
    {
            ...
            ResizeType resizeType = ResizeType.LongestSide, bool ensureSizePositive = true)
        {
            ...
            //we invoke Math.Round to ensure that no white background is rendered - https://www.SmiMarketplace.com/boards/topic/40616/image-resizing-bug
            return new Size((int)Math.Round(width), (int)Math.Round(height));
        }
        ...
        protected virtual void DeletePictureThumbs(Picture picture)
        {
            ...
            var currentFiles = _fileProvider.GetFiles(_fileProvider.GetAbsolutePath(SmiMediaDefaults.ImageThumbsPath), filter, false);
        ...
        {
            var thumbsDirectoryPath = _fileProvider.GetAbsolutePath(SmiMediaDefaults.ImageThumbsPath);

            if (_mediaSettings.MultipleThumbDirectories)
            {
                //get the first two letters of the file name
                var fileNameWithoutExtension = _fileProvider.GetFileNameWithoutExtension(thumbFileName);
                if (fileNameWithoutExtension != null && fileNameWithoutExtension.Length > SmiMediaDefaults.MultipleThumbDirectoriesLength)
                {
                    var subDirectoryName = fileNameWithoutExtension.Substring(0, SmiMediaDefaults.MultipleThumbDirectoriesLength);
                    thumbsDirectoryPath = _fileProvider.GetAbsolutePath(SmiMediaDefaults.ImageThumbsPath, subDirectoryName);
