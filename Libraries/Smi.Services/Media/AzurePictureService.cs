...
namespace Smi.Services.Media
{
    ...
    public partial class AzurePictureService : PictureService
    {
        ...
        protected void OneTimeInit(SmiConfig config)
        {
            if (_isInitialized)
                return;
        ...
        protected virtual async Task DeletePictureThumbsAsync(Picture picture)
        {
            ...
            _staticCacheManager.RemoveByPrefix(SmiMediaDefaults.ThumbsExistsPrefixCacheKey);
        }
        ...
        protected virtual async Task<bool> GeneratedThumbExistsAsync(string thumbFilePath, string thumbFileName)
        {
            try
            {
                var key = _cacheKeyService.PrepareKeyForDefaultCache(SmiMediaDefaults.ThumbExistsCacheKey, thumbFileName);
