...
namespace Smi.Services.Customers
{
    ...
    public partial class CustomerRegistrationService : ICustomerRegistrationService
    {
        ...
        public virtual CustomerRegistrationResult RegisterCustomer(CustomerRegistrationRequest request)
        {
            ...
            switch (request.PasswordFormat)
            {
                ...
                case PasswordFormat.Encrypted:
                    ...
                    break;
                case PasswordFormat.Hashed:
                    var saltKey = _encryptionService.CreateSaltKey(SmiCustomerServicesDefaults.PasswordSaltKeySize);
            ...
            if (registeredRole == null)
                throw new SmiException("'Registered' role could not be loaded");
            ...
            {
                var guestRole = _customerService.GetCustomerRoleBySystemName(SmiCustomerDefaults.GuestsRoleName);
        ...
        public virtual void SetEmail(Customer customer, string newEmail, bool requireValidation)
        {
            ...
            if (newEmail == null)
                throw new SmiException("Email cannot be null");

            newEmail = newEmail.Trim();
            var oldEmail = customer.Email;

            if (!CommonHelper.IsValidEmail(newEmail))
                throw new SmiException(_localizationService.GetResource("Account.EmailUsernameErrors.NewEmailIsNotValid"));

            if (newEmail.Length > 100)
                throw new SmiException(_localizationService.GetResource("Account.EmailUsernameErrors.EmailTooLong"));

            var customer2 = _customerService.GetCustomerByEmail(newEmail);
            if (customer2 != null && customer.Id != customer2.Id)
                throw new SmiException(_localizationService.GetResource("Account.EmailUsernameErrors.EmailAlreadyExists"));
            ...
            if (requireValidation)
            {
                ...
                _genericAttributeService.SaveAttribute(customer, SmiCustomerDefaults.EmailRevalidationTokenAttribute, Guid.NewGuid().ToString());
        ...
        public virtual void SetUsername(Customer customer, string newUsername)
        {
            ...
            if (!_customerSettings.UsernamesEnabled)
                throw new SmiException("Usernames are disabled");

            newUsername = newUsername.Trim();

            if (newUsername.Length > SmiCustomerServicesDefaults.CustomerUsernameLength)
                throw new SmiException(_localizationService.GetResource("Account.EmailUsernameErrors.UsernameTooLong"));

            var user2 = _customerService.GetCustomerByUsername(newUsername);
            if (user2 != null && customer.Id != user2.Id)
                throw new SmiException(_localizationService.GetResource("Account.EmailUsernameErrors.UsernameAlreadyExists"));
