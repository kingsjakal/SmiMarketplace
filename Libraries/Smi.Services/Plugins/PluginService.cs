...
namespace Smi.Services.Plugins
{
    ...
    public partial class PluginService : IPluginService
    {
        ...
        public virtual string GetPluginLogoUrl(PluginDescriptor pluginDescriptor)
        {
            ...
            var logoExtension = SmiPluginDefaults.SupportedLogoImageExtensions
                .FirstOrDefault(ext => _fileProvider.FileExists(_fileProvider.Combine(pluginDirectory, $"{SmiPluginDefaults.LogoFileName}.{ext}")));
            if (string.IsNullOrWhiteSpace(logoExtension))
                return null;

            var storeLocation = _webHelper.GetStoreLocation();
            var logoUrl = $"{storeLocation}{SmiPluginDefaults.PathName}/" +
                $"{_fileProvider.GetDirectoryNameOnly(pluginDirectory)}/{SmiPluginDefaults.LogoFileName}.{logoExtension}";
            ...
            if (checkDependencies)
            {
                ...
                if (descriptor.DependsOn?.Any() ?? false)
                {
                    ...
                    if (dependsOn.Any())
                    {
                        ...
                        throw new SmiException(errorMessage);
                    }
                }
            }
