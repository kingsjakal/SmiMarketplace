    ...
    public class UploadService : IUploadService
    {
        ...
        protected virtual IList<UploadedItem> GetUploadedItems(string archivePath)
        {
            ...
            var uploadedItemsFileEntry = archive.Entries
                .FirstOrDefault(entry => entry.Name.Equals(SmiPluginDefaults.UploadedItemsFileName, StringComparison.InvariantCultureIgnoreCase)
        ...
        protected virtual IDescriptor UploadSingleItem(string archivePath)
        {
            ...
            var pluginsDirectory = _fileProvider.MapPath(SmiPluginDefaults.Path);

            //get path to the themes directory
            var themesDirectory = string.Empty;
            if (!string.IsNullOrEmpty(SmiPluginDefaults.ThemesPath))
                themesDirectory = _fileProvider.MapPath(SmiPluginDefaults.ThemesPath);
            ...
            using (var archive = ZipFile.OpenRead(archivePath))
            {
                ...
                if (rootDirectories.Count != 1)
                {
                    throw new Exception("The archive should contain only one root plugin or theme directory. " +
                        ...
                        $"To upload multiple items, the archive should have the '{SmiPluginDefaults.UploadedItemsFileName}' file in the root");
                }
                ...
                foreach (var entry in archive.Entries)
                {
                    ...
                    var isPluginDescriptor = entry.FullName
                        .Equals($"{uploadedItemDirectoryName}/{SmiPluginDefaults.DescriptionFileName}", StringComparison.InvariantCultureIgnoreCase);

                    //or whether it's a theme descriptor
                    var isThemeDescriptor = entry.FullName
                        .Equals($"{uploadedItemDirectoryName}/{SmiPluginDefaults.ThemeDescriptionFileName}", StringComparison.InvariantCultureIgnoreCase);
                    ...
                    if (isPluginDescriptor)
                    {
                        ...
                        if (!((PluginDescriptor)descriptor).SupportedVersions.Contains(SmiVersion.CurrentVersion))
                            throw new Exception($"This plugin doesn't support the current version - {SmiVersion.CurrentVersion}");
                    }
        ...
        protected virtual IList<IDescriptor> UploadMultipleItems(string archivePath, IList<UploadedItem> uploadedItems)
        {
            ...
            using (var archive = ZipFile.OpenRead(archivePath))
            {
                foreach (var item in uploadedItems)
                {
                    ...
                    //ensure that the current version of SmiMarketplace is supported
                    if (!item.SupportedVersions?.Contains(SmiVersion.CurrentVersion) ?? true)
                        continue;
                    ...
                    if (item.Type == UploadedItemType.Plugin)
                        descriptorPath = $"{itemPath}{SmiPluginDefaults.DescriptionFileName}";

                    if (item.Type == UploadedItemType.Theme && !string.IsNullOrEmpty(SmiPluginDefaults.ThemeDescriptionFileName))
                        descriptorPath = $"{itemPath}{SmiPluginDefaults.ThemeDescriptionFileName}";
                    ...
                    //ensure that the plugin current version is supported
                    if (descriptor is PluginDescriptor pluginDescriptor && !pluginDescriptor.SupportedVersions.Contains(SmiVersion.CurrentVersion))
                        continue;
        ...
        public virtual IList<IDescriptor> UploadPluginsAndThemes(IFormFile archivefile)
        {
            ...
            try
            {
                ...
                var tempDirectory = _fileProvider.MapPath(SmiPluginDefaults.UploadsTempPath);
                ...
                //you can find a sample of such descriptive file in Libraries\Smi.Core\Plugins\Samples\
        ...
        public virtual void UploadIconsArchive(IFormFile archivefile)
        {
            ...
            try
            {
                ...
                var storeIconsPath = _fileProvider.GetAbsolutePath(string.Format(SmiCommonDefaults.FaviconAndAppIconsPath, _storeContext.ActiveStoreScopeConfiguration));
        ...
        public virtual void UploadFavicon(IFormFile favicon)
        {
            ...
            var storeFaviconPath = _fileProvider.GetAbsolutePath(string.Format(SmiCommonDefaults.FaviconAndAppIconsPath, _storeContext.ActiveStoreScopeConfiguration));
        ...
        public virtual void UploadLocalePattern()
        {
            ...
            try
            {
                ...
                var ziplocalePatternPath = getPath(SmiCommonDefaults.LocalePatternPath, SmiCommonDefaults.LocalePatternArchiveName);
                if (!_fileProvider.GetFileExtension(ziplocalePatternPath)?.Equals(".zip", StringComparison.InvariantCultureIgnoreCase) ?? true)
                    throw new Exception($"Archive '{SmiCommonDefaults.LocalePatternArchiveName}' to retrieve localization patterns not found.");
                ...
                if (!(checkDirectoryExists(SmiCommonDefaults.LocalePatternPath, currentCulture.Name) || 
                    checkDirectoryExists(SmiCommonDefaults.LocalePatternPath, currentCulture.TwoLetterISOLanguageName)))
                {
                    var cultureToUse = string.Empty;

                    //3. Unpack the archive into a temporary folder
                    ZipFile.ExtractToDirectory(ziplocalePatternPath, getPath(SmiCommonDefaults.LocalePatternPath, tempFolder));

                    //4. Search in the temp unpacked archive a folder with locales by culture
                    var sourcelocalePath = _fileProvider.Combine(getPath(SmiCommonDefaults.LocalePatternPath, tempFolder), currentCulture.Name);
                    if (_fileProvider.DirectoryExists(sourcelocalePath))
                    {
                        cultureToUse = currentCulture.Name;
                    }

                    var sourcelocaleISOPath = _fileProvider.Combine(getPath(SmiCommonDefaults.LocalePatternPath, tempFolder), currentCulture.TwoLetterISOLanguageName);
                    ...
                    {
                        var destlocalePath = getPath(SmiCommonDefaults.LocalePatternPath, cultureToUse);
            ...
            finally
            {
                ...
                if (checkDirectoryExists(SmiCommonDefaults.LocalePatternPath, tempFolder))
                    _fileProvider.DeleteDirectory(getPath(SmiCommonDefaults.LocalePatternPath, tempFolder));
            }
        }
        ...
        public class UploadedItem
        {
            ...
            /// <summary>
            /// Gets or sets supported versions of SmiMarketplace
            /// </summary>
