...
namespace Smi.Services.Plugins
{
    ...
    public partial class PluginsInfo : IPluginsInfo
    {
        ...
        protected virtual IList<string> GetObsoleteInstalledPluginNames()
        {
            ...
            var filePath = _fileProvider.MapPath(SmiPluginDefaults.InstalledPluginsFilePath);
            if (!_fileProvider.FileExists(filePath))
            {
                //if not, try to parse the file that was used in previous SmiMarketplace versions
                filePath = _fileProvider.MapPath(SmiPluginDefaults.ObsoleteInstalledPluginsFilePath);
                if (!_fileProvider.FileExists(filePath))
        ...
        {
            //save the file
            var filePath = _fileProvider.MapPath(SmiPluginDefaults.PluginsInfoFilePath);
