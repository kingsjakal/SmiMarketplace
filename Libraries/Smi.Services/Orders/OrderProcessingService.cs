    ...
    public partial class OrderProcessingService : IOrderProcessingService
    {
        ...
        protected virtual PlaceOrderContainer PreparePlaceOrderDetails(ProcessPaymentRequest processPaymentRequest)
        {
            ...
            if (_customerService.IsGuest(details.Customer) && !_orderSettings.AnonymousCheckoutAllowed)
                throw new SmiException("Anonymous checkout is not allowed");

            //customer currency
            var currencyTmp = _currencyService.GetCurrencyById(
                _genericAttributeService.GetAttribute<int>(details.Customer, SmiCustomerDefaults.CurrencyIdAttribute, processPaymentRequest.StoreId));
            ...
            details.CustomerLanguage = _languageService.GetLanguageById(
                _genericAttributeService.GetAttribute<int>(details.Customer, SmiCustomerDefaults.LanguageIdAttribute, processPaymentRequest.StoreId));
            if (details.CustomerLanguage == null || !details.CustomerLanguage.Published)
                details.CustomerLanguage = _workContext.WorkingLanguage;

            //billing address
            if (details.Customer.BillingAddressId is null)
                throw new SmiException("Billing address is not provided");
            ...
            if (!CommonHelper.IsValidEmail(billingAddress?.Email))
                throw new SmiException("Email is not valid");
            ...
            if (_countryService.GetCountryByAddress(details.BillingAddress) is Country billingCountry && !billingCountry.AllowsBilling)
                throw new SmiException($"Country '{billingCountry.Name}' is not allowed for billing");

            //checkout attributes
            details.CheckoutAttributesXml = _genericAttributeService.GetAttribute<string>(details.Customer, SmiCustomerDefaults.CheckoutAttributes, processPaymentRequest.StoreId);
            ...
            if (!details.Cart.Any())
                throw new SmiException("Cart is empty");

            //validate the entire shopping cart
            var warnings = _shoppingCartService.GetShoppingCartWarnings(details.Cart, details.CheckoutAttributesXml, true);
            if (warnings.Any())
                throw new SmiException(warnings.Aggregate(string.Empty, (current, next) => $"{current}{next};"));
            ...
            foreach (var sci in details.Cart)
            {
                ...
                if (sciWarnings.Any())
                    throw new SmiException(sciWarnings.Aggregate(string.Empty, (current, next) => $"{current}{next};"));
            }
            ...
            if (!ValidateMinOrderSubtotalAmount(details.Cart))
            {
                ...
                throw new SmiException(string.Format(_localizationService.GetResource("Checkout.MinOrderSubtotalAmount"),
            ...
            if (!ValidateMinOrderTotalAmount(details.Cart))
            {
                ...
                throw new SmiException(string.Format(_localizationService.GetResource("Checkout.MinOrderTotalAmount"),
                    _priceFormatter.FormatPrice(minOrderTotalAmount, true, false)));
            }

            //tax display type
            if (_taxSettings.AllowCustomersToSelectTaxDisplayType)
                details.CustomerTaxDisplayType = (TaxDisplayType)_genericAttributeService.GetAttribute<int>(details.Customer, SmiCustomerDefaults.TaxDisplayTypeIdAttribute, processPaymentRequest.StoreId);
            else
            ...
            if (_shoppingCartService.ShoppingCartRequiresShipping(details.Cart))
            {
                var pickupPoint = _genericAttributeService.GetAttribute<PickupPoint>(details.Customer,
                    SmiCustomerDefaults.SelectedPickupPointAttribute, processPaymentRequest.StoreId);
                ...
                else
                {
                    if (details.Customer.ShippingAddressId == null)
                        throw new SmiException("Shipping address is not provided");
                    ...
                    if (_countryService.GetCountryByAddress(details.ShippingAddress) is Country shippingCountry && !shippingCountry.AllowsShipping)
                        throw new SmiException($"Country '{shippingCountry.Name}' is not allowed for shipping");
                }

                var shippingOption = _genericAttributeService.GetAttribute<ShippingOption>(details.Customer,
                    SmiCustomerDefaults.SelectedShippingOptionAttribute, processPaymentRequest.StoreId);
            ...
            if (!orderShippingTotalInclTax.HasValue || !orderShippingTotalExclTax.HasValue)
                throw new SmiException("Shipping total couldn't be calculated");
            ...
            //VAT number
            var customerVatStatus = (VatNumberStatus)_genericAttributeService.GetAttribute<int>(details.Customer, SmiCustomerDefaults.VatNumberStatusIdAttribute);
            if (_taxSettings.EuVatEnabled && customerVatStatus == VatNumberStatus.Valid)
                details.VatNumber = _genericAttributeService.GetAttribute<string>(details.Customer, SmiCustomerDefaults.VatNumberAttribute);
            ...
            if (!orderTotal.HasValue)
                throw new SmiException("Order total couldn't be calculated");
            ...
            if (!string.IsNullOrEmpty(recurringCyclesError))
                throw new SmiException(recurringCyclesError);
        ...
        protected virtual PlaceOrderContainer PrepareRecurringOrderDetails(ProcessPaymentRequest processPaymentRequest)
        {
            ...
            if (details.InitialOrder.BillingAddressId == 0)
                throw new SmiException("Billing address is not available");
            ...
            if (details.InitialOrder.ShippingStatus != ShippingStatus.ShippingNotRequired)
            {
                ...
                if (!details.PickupInStore)
                {
                    if (!details.InitialOrder.ShippingAddressId.HasValue || !(_addressService.GetAddressById(details.InitialOrder.ShippingAddressId.Value) is Address shippingAddress))
                        throw new SmiException("Shipping address is not available");
        ...
        protected virtual ProcessPaymentResult GetProcessPaymentResult(ProcessPaymentRequest processPaymentRequest, PlaceOrderContainer details)
        {
            ...
            if (!skipPaymentWorkflow)
            {
                var paymentMethod = _paymentPluginManager.LoadPluginBySystemName(processPaymentRequest.PaymentMethodSystemName)
                    ?? throw new SmiException("Payment method couldn't be loaded");

                //ensure that payment method is active
                if (!_paymentPluginManager.IsPluginActive(paymentMethod))
                    throw new SmiException("Payment method is not active");
                ...
                if (details.IsRecurringShoppingCart)
                {
                    ...
                    {
                        case RecurringPaymentType.NotSupported:
                            throw new SmiException("Recurring payments are not supported by selected payment method");
                        case RecurringPaymentType.Manual:
                        case RecurringPaymentType.Automatic:
                            processPaymentResult = _paymentService.ProcessRecurringPayment(processPaymentRequest);
                            break;
                        default:
                            throw new SmiException("Not supported recurring payment type");
                    }
                }
                else
                    //standard cart
        ...
        public virtual PlaceOrderResult PlaceOrder(ProcessPaymentRequest processPaymentRequest)
        {
            ...
            try
            {
                ...
                if (processPaymentResult == null)
                    throw new SmiException("processPaymentResult is not available");
        ...
        public virtual IEnumerable<string> ProcessNextRecurringPayment(RecurringPayment recurringPayment, ProcessPaymentResult paymentResult = null)
        {
            ...
            try
            {
                if (!recurringPayment.IsActive)
                    throw new SmiException("Recurring payment is not active");

                var initialOrder = _orderService.GetOrderById(recurringPayment.InitialOrderId);
                if (initialOrder == null)
                    throw new SmiException("Initial order could not be loaded");

                var customer = _customerService.GetCustomerById(initialOrder.CustomerId);
                if (customer == null)
                    throw new SmiException("Customer could not be loaded");

                if (GetNextPaymentDate(recurringPayment) is null)
                    throw new SmiException("Next payment date could not be calculated");
                ...
                if (!skipPaymentWorkflow)
                {
                    ...
                    {
                        RecurringPaymentType.NotSupported => throw new SmiException("Recurring payments are not supported by selected payment method"),
                        ...
                        _ => throw new SmiException("Not supported recurring payment type"),
                    };
                }
                else
        ...
        public virtual void CancelOrder(Order order, bool notifyCustomer)
        {
            ...
            if (!CanCancelOrder(order))
                throw new SmiException("Cannot do cancel for order.");
        ...
        public virtual IList<string> Capture(Order order)
        {
            ...
            if (!CanCapture(order))
                throw new SmiException("Cannot do capture for order.");
        ...
        public virtual void MarkOrderAsPaid(Order order)
        {
            ...
            if (!CanMarkOrderAsPaid(order))
                throw new SmiException("You can't mark this order as paid");
        ...
        public virtual IList<string> Refund(Order order)
        {
            ...
            if (!CanRefund(order))
                throw new SmiException("Cannot do refund for order.");
        ...
        public virtual void RefundOffline(Order order)
        {
            ...
            if (!CanRefundOffline(order))
                throw new SmiException("You can't refund this order");
        ...
        public virtual IList<string> PartiallyRefund(Order order, decimal amountToRefund)
        {
            ...
            if (!CanPartiallyRefund(order, amountToRefund))
                throw new SmiException("Cannot do partial refund for order.");
        ...
        public virtual void PartiallyRefundOffline(Order order, decimal amountToRefund)
        {
            ...
            if (!CanPartiallyRefundOffline(order, amountToRefund))
                throw new SmiException("You can't partially refund (offline) this order");
        ...
        public virtual IList<string> Void(Order order)
        {
            ...
            if (!CanVoid(order))
                throw new SmiException("Cannot do void for order.");
        ...
        public virtual void VoidOffline(Order order)
        {
            ...
            if (!CanVoidOffline(order))
                throw new SmiException("You can't void this order");
        ...
        public virtual void ReOrder(Order order)
        {
            ...
            _genericAttributeService.SaveAttribute(customer, SmiCustomerDefaults.CheckoutAttributes, order.CheckoutAttributesXml, order.StoreId);
        }
        ...
        public virtual DateTime? GetNextPaymentDate(RecurringPayment recurringPayment)
        {
            ...
            if (historyCollection.Any())
            {
                result = recurringPayment.CyclePeriod switch
                {
                    ...
                    _ => throw new SmiException("Not supported cycle period"),
                };
            }
            else
            {
