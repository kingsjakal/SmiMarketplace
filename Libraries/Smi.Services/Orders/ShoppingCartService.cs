...
namespace Smi.Services.Orders
{
    ...
    public partial class ShoppingCartService : IShoppingCartService
    {
            ...
            bool ensureOnlyActiveCheckoutAttributes = false)
        {
                ...
                shoppingCartItem.ShoppingCartType == ShoppingCartType.ShoppingCart)
            {
                ...
                var checkoutAttributesXml =
                    _genericAttributeService.GetAttribute<string>(customer, SmiCustomerDefaults.CheckoutAttributes,
                        storeId);
                checkoutAttributesXml =
                    _checkoutAttributeParser.EnsureOnlyActiveAttributes(checkoutAttributesXml, cart);
                _genericAttributeService.SaveAttribute(customer, SmiCustomerDefaults.CheckoutAttributes,
            ...
            int storeId = 0, int? productId = null, DateTime? createdFromUtc = null, DateTime? createdToUtc = null)
        {
            ...
            var key = _cacheKeyService.PrepareKeyForShortTermCache(SmiOrderDefaults.ShoppingCartCacheKey, customer, shoppingCartType, storeId, productId, createdFromUtc, createdToUtc);
        ...
        public virtual void MigrateShoppingCart(Customer fromCustomer, Customer toCustomer, bool includeCouponCodes)
        {
            ...
            var checkoutAttributesXml = _genericAttributeService.GetAttribute<string>(fromCustomer, SmiCustomerDefaults.CheckoutAttributes, _storeContext.CurrentStore.Id);
            _genericAttributeService.SaveAttribute(toCustomer, SmiCustomerDefaults.CheckoutAttributes, checkoutAttributesXml, _storeContext.CurrentStore.Id);
        }
            ...
            out int cycleLength, out RecurringProductCyclePeriod cyclePeriod, out int totalCycles)
        {
            ...
            foreach (var sci in shoppingCart)
            {
                ...
                if (product == null)
                {
                    throw new SmiException($"Product (Id={sci.ProductId}) cannot be loaded");
                }
