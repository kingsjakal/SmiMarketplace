    ...
    public partial class OrderTotalCalculationService : IOrderTotalCalculationService
    {
            ...
            bool? useRewardPoints, Customer customer, decimal orderTotal)
        {
            ...
            if (!useRewardPoints.HasValue)
                useRewardPoints = _genericAttributeService.GetAttribute<bool>(customer, SmiCustomerDefaults.UseRewardPointsDuringCheckoutAttribute, _storeContext.CurrentStore.Id);
            ...
            out SortedDictionary<decimal, decimal> taxRates)
        {
            ...
            if (customer != null)
            {
                var checkoutAttributesXml = _genericAttributeService.GetAttribute<string>(customer, SmiCustomerDefaults.CheckoutAttributes, _storeContext.CurrentStore.Id);
            ...
            out decimal taxRate, out List<Discount> appliedDiscounts)
        {
            ...
            if (customer != null)
                shippingOption = _genericAttributeService.GetAttribute<ShippingOption>(customer, SmiCustomerDefaults.SelectedShippingOptionAttribute, _storeContext.CurrentStore.Id);
            ...
            else
            {
                ...
                if (!shippingRateComputationMethods.Any() && !_shippingSettings.AllowPickupInStore)
                    throw new SmiException("Shipping rate computation method could not be loaded");
