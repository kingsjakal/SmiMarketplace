...
namespace Smi.Core
{
    ...
    public partial class WebHelper : IWebHelper
    {
        ...
        protected virtual bool TryWriteWebConfig()
        {
            try
            {
                _fileProvider.SetLastWriteTimeUtc(_fileProvider.MapPath(SmiInfrastructureDefaults.WebConfigPath), DateTime.UtcNow);
                return true;
            }
            catch
            {
        ...
        public virtual string GetCurrentIpAddress()
        {
            ...
            try
            {
                ...
                if (_httpContextAccessor.HttpContext.Request.Headers != null)
                {
                    ...
                    var forwardedHttpHeaderKey = SmiHttpDefaults.XForwardedForHeader;
        ...
        public virtual bool IsCurrentConnectionSecured()
        {
            ...
            if (_hostingConfig.UseHttpClusterHttps)
                return _httpContextAccessor.HttpContext.Request.Headers[SmiHttpDefaults.HttpClusterHttpsHeader].ToString().Equals("on", StringComparison.OrdinalIgnoreCase);

            //use HTTP_X_FORWARDED_PROTO?
            if (_hostingConfig.UseHttpXForwardedProto)
                return _httpContextAccessor.HttpContext.Request.Headers[SmiHttpDefaults.HttpXForwardedProtoHeader].ToString().Equals("https", StringComparison.OrdinalIgnoreCase);
        ...
        public virtual void RestartAppDomain(bool makeRedirect = false)
        {
            ...
            if (!success)
            {
                throw new SmiException("SmiMarketplace needs to be restarted due to a configuration change, but was unable to do so." + Environment.NewLine +
        ...
        public virtual bool IsPostBeingDone
        {
            get
            {
                if (_httpContextAccessor.HttpContext.Items[SmiHttpDefaults.IsPostBeingDoneRequestItem] == null)
                    return false;

                return Convert.ToBoolean(_httpContextAccessor.HttpContext.Items[SmiHttpDefaults.IsPostBeingDoneRequestItem]);
            }

            set => _httpContextAccessor.HttpContext.Items[SmiHttpDefaults.IsPostBeingDoneRequestItem] = value;
        }
